defmodule AC.Eight do
  ...
  defmodule Grid do
    ...
    def reduce(grid, acc, fun) do
      Tuple.to_list(grid)
      |> Enum.flat_map(fn row -> Tuple.to_list(row) end)
      |> Enum.reduce(acc, fn x, acc -> fun.(x, acc) end)
    end
  end
  ...
  @spec test_output_part2 :: 8
  def test_output_part2 do
    8
  end

  @spec problem1(String.t()) :: String.t()
      ...
      fn x ->
        ...
        |> Enum.reduce(:hidden, fn x, acc ->
          cond do
            x == :visible -> :visible
            x == :hidden -> acc
            true -> raise "state error"
          end
        end)
      end
    )
    |> Grid.reduce(0, fn x, acc ->
      cond do
        x == :hidden -> acc
        x == :visible -> acc + 1
      end
